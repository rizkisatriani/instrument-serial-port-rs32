<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	class Cekbatchpan_controller extends CI_Controller {
    private $cat;
    private $menu;
    private $submenu;
    private $module;
    ...
    function do_action(){
        ...
        switch($action){
            case 'showdata':
              //$data['analisa'] = $this->analisa_model->show_analisa_verifikasi($param='batch'); 
			...
			case 'open_detil_analisa':
                ...
                $data['material'] = $this->analisa_model->get_material($param = 'batch');
			...
			case 'open_form_grid_analisa':
                ...
                $data['tgl'] = $tgl;
                $data['jam'] = $jam; 
                $data['chek'] = $this->analisa_model->chek_verifikasi($tgl,$jam,'batch');
                
                $data['analisa'] = $this->analisa_model->get_process_verifikasi_analisa_tmp($tgl,$jam,$jam2,$param='batch'); 
                $data['material'] = $this->analisa_model->get_material($param = 'batch');  
            ...
            case 'showdatatable':
                ...
                $data['analisa'] = $this->analisa_model->show_analisa_verifikasi($param='batch',$tanggal);
