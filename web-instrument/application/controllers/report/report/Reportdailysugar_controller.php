...
class Reportdailysugar_controller extends CI_Controller {
    ...
    function __construct()
    {
        ...
        $this->load->model('reportdailysugar_model');
    ...
    function export_pdf(){ 
        ...
        $this->load->vars( $var );  
        $data['data'] = $this->reportdailysugar_model->get_data($tanggal_awal);    
        $data['data_count'] = $this->reportdailysugar_model->get_data_count($tanggal_awal); 
        $query = $this->db->query("SELECT id_jam AS no_strike FROM verifikasi_detil WHERE tanggal ='".$tanggal_awal."' AND param='sugar' GROUP BY id_jam");
        $data['material'] = $query->result();  
        $data['tanggal']=$tanggal_awal; 
        $this->load->view( $url. '/cetak_report_sugar_pro_daily',$data);
