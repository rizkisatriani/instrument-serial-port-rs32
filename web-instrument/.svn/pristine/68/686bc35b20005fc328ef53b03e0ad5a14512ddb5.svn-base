...
class Reportdailymolasses_controller extends CI_Controller {
    ...
    function __construct()
    {
        ...
        $this->load->model('reportdailymolasses_model');
    ...
    function export_pdf(){
                ...
                $this->load->vars( $var );  
                $data['data'] = $this->reportdailymolasses_model->get_data($tanggal_awal);  
                $query = $this->db->query("SELECT id_jam   FROM verifikasi_detil WHERE tanggal='$tanggal_awal' and param ='mollasses' GROUP BY id_jam");
                $data['material'] = $query->result();
                $data['data_count'] = $this->reportdailymolasses_model->get_data_count($tanggal_awal);     
                $data['tanggal']=$tanggal_awal; 
                $this->load->view( $url. '/cetak_report_mollasses_daily',$data);
