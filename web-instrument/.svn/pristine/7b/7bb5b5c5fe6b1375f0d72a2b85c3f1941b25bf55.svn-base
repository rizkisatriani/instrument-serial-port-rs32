<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	class Analisacondensate_controller extends CI_Controller {
    private $cat;
    private $menu;
    private $submenu;
    private $module;
	...
	function simpan_analisa(){
		...
		$simpan = $this->analisa_model->simpan_analisa_condensate($paramArray=true);
		//print_r( array( $this->input->post('parameter') ));

		if ($simpan){ echo "OK"; };
    }
    
    function update_analisa() {
        $id = $this->input->post('no_analisa');
	...
	function get_brix_clr(){
		...
		$param = 'P051';
		$data = $this->analisa_model->get_analisa_by_parameter_and_item($param);
    ...
    function simpan_sementara(){
		...
		$simpan = $this->analisa_model->simpan_sementara_condensate();
