<?php 
    ...
    foreach($material as $val){
        ...
        foreach($analisa as $val_analisa){
            $strike_analisa = $val_analisa->no_strike;  
            $jam = $val_analisa->id_jam;
...
<table  class="table striped table-border cell-border row-border cell-hover subcompact">
    <thead >
        <tr class="bg-orange" >
            ...
            <th width='5%' style='text-align:center;' >Shift</th>
...
<div class="row border-top bd-gray">
    ...
    <?php 
        $tgl_str="'$tgl'"; 
    if(count($analisa)>0){
        echo '<div class="cell-sm-full cell-md-two-third cell-lg-3"><button class="button primary" onclick="simpanVerifikasi('.$tgl_str.','.$jam.')"><span class="mif-done_all"></span> Simpan</button></div>
        <div class="cell-sm-full cell-md-half cell-lg-3"><button class="button alert" onclick="batalVerifikasi('.$tgl_str.','.$jam.')"><span class="mif-cross"></span> Batal</button></div>';
    }else{
