<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	class Analisaph_controller extends CI_Controller {
    private $cat;
    private $menu;
    private $submenu;
    private $module;
    ...
    function do_action(){
        ...
        switch($action){
            case 'showdata':
                ...
                $this->load->view( $this->url . '/show_daftar_analisaph',$data);
            break;
            case 'open_form_analisa':
                $this->load->view( $this->url . '/show_form_analisaph',$data);
            break;
			case 'open_detil_analisa':
				$id = $param_data[1];
				$data['analisa'] = $this->analisa_model->get_analisa_by_id($id);
				$this->load->view( $this->url . '/show_detil_analisaph',$data);
			break;
            case 'showdatatable':
	...
	function simpan_analisa(){
		...
		$simpan = $this->analisa_model->simpan_analisa();
