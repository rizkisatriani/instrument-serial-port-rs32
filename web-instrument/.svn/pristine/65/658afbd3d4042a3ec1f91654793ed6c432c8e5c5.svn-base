<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	class Ceksugarproduct_controller extends CI_Controller {
    private $cat;
    private $menu;
    private $submenu;
    private $module;
    ...
    function do_action(){
        ...
        switch($action){
            case 'showdata':
              //$data['analisa'] = $this->analisa_model->show_analisa_verifikasi($param='sugar'); 
			...
			case 'open_detil_analisa':
                ...
                $query = $this->db->query("SELECT no_strike FROM verifikasi_detil WHERE id_verifikasi ='$id' GROUP BY no_strike");
			...
			case 'open_form_grid_analisa':
                ...
                $data['tgl'] = $tgl; 
                $data['shift'] = $jam;
                $data['chek'] = $this->analisa_model->chek_verifikasi_sugar($tgl,'sugar'); 
                $data['analisa'] = $this->analisa_model->get_process_verifikasi_analisa_tmp($tgl,$jam,$jam2,$param='sugar');
                $query = $this->db->query("SELECT COALESCE(no_strike,0) as no_strike FROM hasil_analisa WHERE tanggal='$tgl' AND id_material = '39' AND id_jam = '$jam' GROUP BY no_strike");
                $data['material'] = $query->result();  
            ...
            case 'showdatatable':
                ...
                $data['analisa'] = $this->analisa_model->show_analisa_verifikasi($param='sugar',$tanggal);
