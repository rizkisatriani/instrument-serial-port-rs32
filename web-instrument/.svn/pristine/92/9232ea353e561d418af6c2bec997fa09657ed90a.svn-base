<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	class Cekevaporator_controller extends CI_Controller {
    private $cat;
    private $menu;
    private $submenu;
    private $module;
    ...
    function do_action(){
        ...
        switch($action){
            case 'showdata':
              //$data['analisa'] = $this->analisa_model->show_analisa_verifikasi($param='evap');
			...
			case 'open_detil_analisa':
                ...
                $data['material'] = $this->analisa_model->get_material($param = 'evap');
			...
			case 'open_form_grid_analisa':
                ...
                $data['tgl'] = $tgl;
                $data['jam'] = $jam; 
                $data['chek'] = $this->analisa_model->chek_verifikasi($tgl,$jam,'evap'); 
                $data['analisa'] = $this->analisa_model->get_process_verifikasi_analisa_tmp($tgl,$jam,$jam2,$param='evap');  
                $data['material'] = $this->analisa_model->get_material($param = 'evap'); 
            ...
            case 'showdatatable':
                ...
                $data['analisa'] = $this->analisa_model->show_analisa_verifikasi($param='evap',$tanggal);
