...
//var_dump($header);
 $p->Cell(20,6,"Material" ,1,0,'C',true);   
 $n=0;
foreach($header_t4 as $val){ 
    switch ($n) {
        case 0:
            $width=37.5;
            break; 
...
switch ($n) {
        case 0:
            $width=25;
        $p->Cell(($width/2),6,'Temp',1,0,'C',true);  
            break; 
        case 1:
            $width=25;
        $p->Cell(($width/2),6,'Temp',1,0,'C',true);  
