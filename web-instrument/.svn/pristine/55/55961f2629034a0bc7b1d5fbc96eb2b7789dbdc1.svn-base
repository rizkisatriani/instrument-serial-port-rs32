<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	class Cekboiler_controller extends CI_Controller {
    private $cat;
    private $menu;
    private $submenu;
    private $module;
    ...
    function do_action(){
        ...
        switch($action){
            case 'showdata':
              //$data['analisa'] = $this->analisa_model->show_analisa_verifikasi($param='boiler'); 
			...
			case 'open_detil_analisa':
                ...
                $data['material'] = $this->analisa_model->get_material($param = 'boiler'); 
                $data['parameter'] = $this->analisa_model->get_parameter($param = 'is_boiler'); 
			...
			case 'open_form_grid_analisa':
                ...
                //$data['chek'] = $this->analisa_model->chek_verifikasi($tgl,$jam,'boiler');
                
                $data['analisa'] = $this->analisa_model->get_process_verifikasi_analisa_tmp($tgl,$jam,$jam2,$param='boiler'); 
                $data['material'] = $this->analisa_model->get_material($param = 'boiler'); 
                $data['parameter'] = $this->analisa_model->get_parameter($param = 'is_boiler');
            ...
            case 'showdatatable':
                ...
                $data['analisa'] = $this->analisa_model->show_analisa_verifikasi($param='boiler',$tanggal);
