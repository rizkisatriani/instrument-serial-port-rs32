<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	class Analisaconductivity_controller extends CI_Controller {
    private $cat;
    private $menu;
    private $submenu;
    private $module;
	...
	function simpan_analisa(){
        ...
        $status = $this->input->post('array');
    
		$simpan = $this->analisa_model->simpan_analisa( $status );

		if ($simpan){ echo "OK"; };
    }
    
    function update_analisa() {
        $nomor = $this->input->post('no_analisa');
        $tgl =  $this->input->post('tanggal');
        $jam =  $this->input->post('jam_analisa');

        $update = $this->analisa_model->update_analisa($nomor, $tgl, $jam);
        if($update) { echo "OK"; };
    }
  }
